#!/usr/bin/env python3
"""Vault guard: config-driven frontmatter, syntax, link, and todo.txt linter.

Reads .scripts/vault.config.json for controlled vocabularies and domain folders,
...
# root-level meta files are not vault notes; they document the vault
...
def check_synthesis(fm):
    ...
    if synth == "generated":
        ...
        if not isinstance(sources, list) or not sources:
            errors.append("synthesized page must cite a non-empty 'sources' list (vault-root-relative paths)")
        else:
            for s in sources:
                if not isinstance(s, str):
                    errors.append(f"source entry must be a path string, got {type(s).__name__}")
                    continue
                if not (VAULT_ROOT / s).exists():
...
def check_links(text, path):
    ...
    for m in INTERNAL_LINK.finditer(text):
        ...
        target = (VAULT_ROOT / raw.lstrip("/")) if raw.startswith("/") else (path.parent / raw)
...
def main(argv):
    ...
    if results:
        ...
        print(f"\n{len(results)} file(s) failed vault validation.")
        auto = sum(1 for _, errs in results for e in errs if classify(e) == "auto")
        if auto:
            print(f"{auto} item(s) are auto-fixable: run  python3 .scripts/validate_vault.py --fix --all")
