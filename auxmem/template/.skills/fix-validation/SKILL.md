---
name: fix-validation
description: Fix auxmem vault validation failures using the tiered protocol (auto, llm, human). Use when validate_vault.py fails, the pre-commit hook rejects a commit, or the user asks to fix vault validation errors.
---

# Fix validation failures

Do not weaken the gate. Fix notes until `python3 .scripts/validate_vault.py --all` passes.

## Diagnose

```bash
python3 .scripts/validate_vault.py --json --all
```
...
- **type** or **domain** not in vocabulary — offer valid options from `.scripts/vault.config.json`
