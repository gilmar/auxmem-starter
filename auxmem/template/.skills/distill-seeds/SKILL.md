---
name: distill-seeds
description: Distill AI provider export staging corpus into seed notes inside an auxmem vault. Use when bootstrapping a vault from seed-staging, provider exports, or conversation history imports.
---

# Distill seeds

Staging corpus lives **outside** the vault in `seed-staging/`. Output is 15–30 current-state seed notes inside the vault.
...
- ADRs in `60-decisions/` from ADR-0002 onward (0001 is the vault itself)
- `80-moc/home-moc.md` plus domain MOCs linking every seed
- `71-log/<today>-vault-bootstrap.md`
- `72-tasks/todo.txt` for open commitments (todo.txt grammar, creation date = today)

## Hard rules

1. Full frontmatter on every note. Run `python3 .scripts/validate_vault.py --all` before finishing.
2. Never copy transcript text verbatim — synthesize in your own words.
3. No sensitive personnel content in this vault. Flag sources in `seed-staging/sensitive-review.md` (outside vault); do not summarize sensitive content.
...
Full importer doc: auxmem-starter `importers/distill-seeds.md` (reference if available outside vault).
