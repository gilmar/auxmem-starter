...
How raw material becomes synthesized knowledge in this vault, intentionally and with provenance. Synthesis is a batch operation you run, not something that happens silently.
...
`05-sources/` is the synthesis queue. Dropping material there does not synthesize it; running the loop does. This keeps synthesis intentional. Silent per-write synthesis is the drift failure mode this vault is designed to avoid.
...
4. For every synthesized page:
   - Cite every source it draws from in `sources:` (vault-root-relative paths, e.g. `05-sources/cube-docs.md` or `10-data-hub/mcp-server.md`).
...
6. `python3 .scripts/gen_mocs.py` then `python3 .scripts/validate_vault.py --all`. Fix violations.
7. Commit.
...
- Never synthesize sensitive personnel content into the vault. If a source contains it, it does not belong in `05-sources/` either. See ARCHITECTURE.md.
