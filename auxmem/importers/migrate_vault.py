#!/usr/bin/env python3
"""migrate_vault.py: import an Obsidian vault into the open-standard vault.
...
Usage:
  migrate_vault.py --src ~/old-vault --dst ~/vault [--map map.json] [--dry-run]
...
# Domain vocabulary is loaded from the target vault's config at runtime
# (see load_vault_domains). These are fallbacks only if no config is passed.
DOMAIN_BY_PREFIX = {}
REPORT_DOMAIN = "reference"


def load_vault_domains(vault_config_path):
    """Read the target vault's config: build prefix->slug map and a report domain."""
...
def main():
    ...
    if not src.is_dir():
        sys.exit(f"source vault not found: {src}")
