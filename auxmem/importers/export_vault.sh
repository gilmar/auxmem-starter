#!/bin/bash
# export_vault.sh: stage 1 of the Obsidian import pipeline.
# Runs obsidian-export to convert the vault to CommonMark, capturing its
# unresolved-link warnings for the restructure report.
#
# Usage: export_vault.sh <obsidian_vault> <export_dir>

set -euo pipefail

SRC="${1:?usage: export_vault.sh <obsidian_vault> <export_dir>}"
OUT="${2:?usage: export_vault.sh <obsidian_vault> <export_dir>}"
...
# default ignore file if the vault has none; gitignore syntax
...
echo "next: python3 restructure_export.py --src $OUT --dst ~/vault --map map.json"
