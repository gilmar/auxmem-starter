"""Upgrade an existing auxmem vault to the current template version.

Per-file policy:
  overwrite  tooling replaced with the new version (old backed up; user edits
             detected via the snapshot and flagged)
  merge      vault.config.json: preserve user values, add new schema keys/options
  merge3     guidance/docs/templates: git 3-way merge (base = vault snapshot,
...
def _vault(dest):
    ...
    if not (dest / ".scripts" / "vault.config.json").exists():
        raise UpgradeError(f"{dest} is not an auxmem vault")
    return dest
...
def _git_merge3(ours: Path, base: Path, theirs: Path):
    ...
    # git merge-file returns >=0 = number of conflicts, <0 = error
            ...
            for k2, v2 in new_val.items():
                if k2 not in out[key]:
                    ...
                    if added2:
                        ...
                        report.append(f"config: added to '{key}.{k2}': {', '.join(map(str, added2))}")
        # scalars that already exist are left as the user's value
    return out
...
def upgrade(dest, force=False):
    ...
    for rel, meta in new_manifest["files"].items():
        ...
        is_new = rel not in old_manifest["files"]
        ...
        if policy == "overwrite":
        ...
        elif policy == "merge":  # vault.config.json
            backup(rel)
            ...
            # A file present on disk is not "new" even if the old manifest lacks
            # it (pre-versioning vault). Only truly absent files are added.
            ...
            if base and base.read_bytes() == new_bytes:
                ...
                continue
            if base is None:
                # no merge base: cannot 3-way. Install new alongside for manual review.
                backup(rel)
    ...
    # note removed managed files (do not delete)
    ...
    # create any folders newly introduced by the merged config (upgrade does
    # not run bootstrap, so do the folder-creation step here)
    merged_cfg = json.loads((dest / ".scripts" / "vault.config.json").read_text(encoding="utf-8"))
    ...
    # refresh snapshot + manifest to the new version
    if snapshot.exists():
    ...
    # regenerate MOCs and validate
    subprocess.run(["python3", ".scripts/gen_mocs.py"], cwd=dest,
                   capture_output=True, text=True)
    val = subprocess.run(["python3", ".scripts/validate_vault.py", "--all"], cwd=dest,
                         capture_output=True, text=True)
...
def _primary_domain(dest):
    cfg = json.loads((dest / ".scripts" / "vault.config.json").read_text(encoding="utf-8"))
    return next(iter(cfg["domains"].values()))
