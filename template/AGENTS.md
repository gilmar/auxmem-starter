...
3. Read `todo.txt` for open tasks.
...
Open tasks in `todo.txt`, archive in `done.txt`. One task per line, no blank lines.
...
Write rules: append new tasks at the end; complete in place; never delete an open task (to drop one, complete it with `+cancelled`); at session close move `x` lines to `done.txt`; done.txt is append-only. Run the validator on todo.txt after editing.

## Session close
1. Complete or update `todo.txt`, move done tasks to `done.txt`.
