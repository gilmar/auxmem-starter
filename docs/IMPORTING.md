...
Two one-time paths, both run from auxmem-starter (not from inside a vault): seed a vault from AI provider exports, and migrate an existing Obsidian vault. Both end with the target vault's own validator as the gate.

Create the target vault first with `auxmem new`. Import writes into an existing vault.

## A. Seed from provider exports (Claude, ChatGPT, Gemini)

Two stages. A deterministic extractor normalizes exports into a staging corpus (outside any vault). Then a CLI agent distills that corpus into validated seed notes inside the vault.
...
```
The extractor auto-detects the provider, writes one file per conversation plus a triage `index.md`, and lives outside the vault (gitignored there).
...
git add -A && git commit -m "seed vault from provider exports"
```
...
The obsidian-export pipeline is primary (better link fidelity, note embeds inlined). A single-script fallback needs no toolchain. Both read the target vault's config, so imported notes validate against that vault's domains.
...
### After either
Read `~/my-work/00-inbox/migration-report.md` before committing. It lists unmapped links (now plain text), files needing manual handling (`00-inbox/import-manual/`), and frontmatter retrofits to review. Unmapped and ambiguous links degrade to plain text, never broken links, so the vault stays valid. `auxmem import-obsidian` runs the vault's validator and MOC generator automatically after a real (non-dry-run) import. Treat the first real import as a rehearsal on a scratch copy of the destination.
