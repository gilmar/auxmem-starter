#!/usr/bin/env python3
"""Corpus guard: config-driven frontmatter, syntax, link, and todo.txt linter.

Reads .scripts/koinome.config.json for controlled vocabularies and domain folders,
so the same script serves any corpus built from this template.

Usage:
  validate_corpus.py file1.md 72-tasks/todo.txt   # validate specific files (pre-commit)
  validate_corpus.py --all               # validate the whole corpus (CI)
...
# root-level meta files are not corpus notes; they document the corpus
...
def resolve_note_target(raw: str, source: Path, root: Path) -> Path:
    """Resolve a markdown internal-link path relative to source and corpus root."""
...
def resolve_source_target(raw: str, root: Path) -> Path:
    """Resolve a synthesis source path (corpus-root-relative, no leading slash)."""
...
def check_synthesis(fm):
    ...
    if synth == "generated":
        ...
        if not isinstance(sources, list) or not sources:
            errors.append("synthesized page must cite a non-empty 'sources' list (corpus-root-relative paths)")
        else:
            for s in sources:
...
def _check_source_path(raw: str) -> list[str]:
    ...
    if Path(raw).is_absolute():
        ...
        return errors
    target = resolve_source_target(raw, CORPUS_ROOT)
    if not resolves_within_root(target, CORPUS_ROOT):
        errors.append(f"sources: path escapes corpus root -> {raw}")
        return errors
...
def check_links(text, path):
    ...
    for m in INTERNAL_LINK.finditer(text):
        ...
        if Path(decoded).is_absolute() and not decoded.startswith("/"):
            ...
            continue
        target = resolve_note_target(raw, path, CORPUS_ROOT)
        if not resolves_within_root(target, CORPUS_ROOT):
            errors.append(f"line {line}: link escapes corpus root -> {raw}")
            continue
        if not target.exists():
...
def classify(err: str) -> str:
    ...
    if "not in controlled vocabulary" in err \
       ...
       or "sources: path escapes corpus root" in err \
       or "sources: absolute path not allowed" in err \
       or "link escapes corpus root" in err \
...
def _collect_files(argv):
    if "--all" in argv:
        return sorted(
            p for p in list(CORPUS_ROOT.rglob("*.md"))
            + [CORPUS_ROOT / TODO_DIR / f for f in TODO_FILES]
            if p.exists()
            and not any(part in SKIP_DIRS for part in p.relative_to(CORPUS_ROOT).parts)
        )
...
def main(argv):
    ...
    if results:
        ...
        print(f"\n{len(results)} file(s) failed corpus validation.")
        auto = sum(1 for _, errs in results for e in errs if classify(e) == "auto")
        if auto:
            print(f"{auto} item(s) are auto-fixable: run  python3 .scripts/validate_corpus.py --fix --all")
