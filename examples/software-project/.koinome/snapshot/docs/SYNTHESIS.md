...
How raw material becomes synthesized knowledge in this corpus, intentionally and with provenance. Synthesis is a batch operation you run, not something that happens silently.
...
The distinction is enforced by the validator (schema and vocab from `.scripts/koinome.config.json`): a synthesized page must declare `synthesis: generated`, cite a non-empty `sources:` list, and carry `review:` and `generated_at`. A page that does not cite its provenance cannot exist. Authored notes must never claim to be generated.

## Why it is a queue, not automatic

`05-sources/` is the synthesis queue. Dropping material there does not synthesize it; running the loop does. This keeps synthesis intentional. Silent per-write synthesis is the drift failure mode this corpus is designed to avoid.
...
4. For every synthesized page:
   - Cite every source it draws from in `sources:` (corpus-root-relative paths, e.g. `05-sources/cube-docs.md` or `<domain-folder>/mcp-server.md` from your config).
...
6. `python3 .scripts/gen_mocs.py` then `python3 .scripts/validate_corpus.py --all`. Fix violations.
7. Commit.
...
- Never synthesize sensitive personnel content into the corpus. If a source contains it, it does not belong in `05-sources/` either. See ARCHITECTURE.md.
