#!/usr/bin/env python3
"""Validated git sync with quarantine for a corpus folder.

State machine:
  1. Resolve corpus root, branch, and remote (env overrides supported).
  2. Acquire a per-repository lock under .git/koinome-sync.lock/.
  3. Stage pending changes; validate the index snapshot before any canonical commit.
  4. On valid pending corpus changes: verified commit, pull --rebase, conformance check, push.
  5. On invalid pending corpus changes: quarantine branch, restore canonical tip, write alert.
...
def _log(msg: str) -> None:
    print(f"[koinome-sync {_iso_now()}] {msg}")
...
def _read_config(root: Path) -> dict:
    return json.loads((root / ".scripts" / "koinome.config.json").read_text(encoding="utf-8"))
...
def resolve_sync_target(corpus_arg: str | None) -> Path:
    if corpus_arg:
        raw = corpus_arg
    elif os.environ.get("CORPUS_ROOT"):
        raw = os.environ["CORPUS_ROOT"]
    else:
        raw = str(Path.cwd())
    root = Path(raw).expanduser().resolve()
    if not root.is_dir():
        raise FileNotFoundError(f"corpus folder not found: {root}")
    if not (root / ".scripts" / "koinome.config.json").is_file():
        raise FileNotFoundError(f"not a corpus (missing .scripts/koinome.config.json): {root}")
    return root


def resolve_remote_branch(root: Path) -> tuple[str, str]:
    branch = os.environ.get("CORPUS_SYNC_BRANCH", "").strip()
    remote = os.environ.get("CORPUS_SYNC_REMOTE", "").strip()
    if not branch:
...
def acquire_lock(root: Path) -> Path | None:
    lock = root / ".git" / "koinome-sync.lock"
    try:
        lock.mkdir()
...
def check_conformance(root: Path) -> tuple[int, str]:
    proc = _run([_PYTHON, ".scripts/check_corpus.py"], cwd=root)
    message = (proc.stdout or proc.stderr).strip()
    return proc.returncode, message
...
def _write_corpus_alert(root: Path, *, filename: str, body: str) -> Path:
    path = root / ".koinome" / "alerts" / filename
    path.parent.mkdir(parents=True, exist_ok=True)
...
def write_invalid_alert(root: Path, branch: str, detail: str, domain: str | None) -> Path:
    ...
    return _write_corpus_alert(root, filename=f"sync-invalid-{ts}.md", body=body)
...
def write_conflict_alert(root: Path, branch: str, domain: str | None) -> Path:
    ...
    return _write_corpus_alert(root, filename=f"sync-conflict-{ts}.md", body=body)
...
def sync_corpus(root: Path) -> int:
    ...
    if lock is None:
        _log("another sync is running for this corpus, skipping")
        return FAIL
