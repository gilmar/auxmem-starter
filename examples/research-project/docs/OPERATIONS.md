...
How to run the corpus day to day.

## The daily loop
1. Start an agent session from the corpus root. The agent reads AGENTS.md, the home MOC, `72-tasks/todo.txt`, and recent logs.
...
```bash
python3 .scripts/check_corpus.py              # validation + MOC freshness
koinome check PATH                             # same, from outside the corpus
python3 .scripts/check_corpus.py --manifest   # also verify managed tooling hashes
python3 .scripts/check_corpus.py --git        # also require a clean working tree
```

To regenerate stale MOCs before committing, use `python3 .scripts/gen_mocs.py` or `koinome doctor PATH` (doctor may write files; check does not).

```bash
python3 .scripts/validate_corpus.py --all         # validation only
python3 .scripts/validate_corpus.py path/to/note.md 72-tasks/todo.txt   # specific files
```
The pre-commit hook validates the staged git index snapshot automatically. To bypass once (rare): `git commit --no-verify`.

## Continuous integration
New corpora include `.github/workflows/koinome-check.yml`. On each push or pull request it installs PyYAML and runs `python3 .scripts/check_corpus.py`. Enable GitHub Actions on your remote to activate it.
...
5. **Invalid pending corpus changes:** quarantine commit on `sync-invalid/<host>/<timestamp>`, push that branch when a remote exists, reset the canonical branch to its prior tip, write an alert (`00-inbox/sync-invalid-*.md` when a domain exists, else `.koinome/alerts/`), exit `3`.
...
- Inspect `00-inbox/sync-invalid-*.md` (or `.koinome/alerts/sync-invalid-*.md`).
...
- An alert appears in `00-inbox/sync-conflict-*.md` (or `.koinome/alerts/`).

To recover, from the corpus:
```bash
...
## Health checks
```bash
python3 .scripts/check_corpus.py           # read-only conformance (CI)
koinome check PATH                          # same, from the Koinome CLI
koinome doctor PATH                         # regenerate MOCs, then validate
python3 .scripts/validate_corpus.py --all  # validation only
