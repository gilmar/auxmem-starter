---
name: koinome-synthesize
description: Run the governed corpus synthesis loop over 05-sources into 85-synthesis entity and concept pages with provenance. Use when synthesizing sources, refreshing stale synthesized pages, or when synthesis_status.py reports queue or staleness.
---
...
3. On every synthesized page set:
   - `synthesis: generated`
   - `sources:` — corpus-root-relative paths (non-empty)
   - `generated_at:` today
   - `review: needed`
   - `type` / `status` from `vocab` in `.scripts/koinome.config.json`
...
6. `python3 .scripts/gen_mocs.py` then `python3 .scripts/validate_corpus.py --all`. Fix failures (see `koinome-fix-validation` skill).
7. Commit.
