---
name: koinome-todo
description: Manage corpus tasks in 72-tasks/todo.txt and done.txt using the todo.txt grammar. Use when adding, completing, prioritizing, or archiving corpus tasks.
---
...
- `+project` uses domain slugs from `.scripts/koinome.config.json`
...
- After edits: `python3 .scripts/validate_corpus.py --all` (validator enforces grammar)

For full session wrap-up, use the `koinome-session-close` skill.
