---
name: koinome-session-close
description: Close a corpus agent session. Completes tasks, archives done items, appends a log entry, regenerates MOCs, validates, and commits. Use at session end or when the user asks to wrap up, close out, or finish a corpus session.
---
...
4. **Validate** — `python3 .scripts/validate_corpus.py --all`. If failures remain, invoke the `koinome-fix-validation` skill.
