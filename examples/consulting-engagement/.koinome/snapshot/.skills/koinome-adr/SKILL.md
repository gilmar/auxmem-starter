---
name: koinome-adr
description: Write or supersede a corpus architecture decision record (ADR) and update the decision log index. Use when recording a durable decision, reversing a prior decision, or when the user asks for an ADR.
---
...
6. `python3 .scripts/gen_mocs.py` then `python3 .scripts/validate_corpus.py --all`. Commit.
