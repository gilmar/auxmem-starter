"""Interactive terminal wizard for `koinome new`.
...
def _template_structural_folders():
    cfg_path = scaffold.TEMPLATE_DIR / ".scripts" / "koinome.config.json"
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    return cfg.get("structural_folders", [])


def _show_preview(name, path):
    print(f"\n{BOLD}this corpus will contain:{RESET}\n")
    print(f"  {DIM}name{RESET}     {name}")
    print(f"  {DIM}path{RESET}     {path}")
    print(
        f"\n  {DIM}subject domains{RESET}  "
        f"{DIM}none yet — koinome-init will define them with you{RESET}"
    )
    print(f"\n  {DIM}shared structure{RESET} (same in every corpus)")
...
def run():
    if not sys.stdin.isatty():
        raise scaffold.ScaffoldError(
            "no interactive terminal; use flags: koinome new --name NAME --path PATH "
    ...
    _step(1, total, "Name Your corpus")
    print(
        f"{DIM}Used as the corpus label, git repo name, and in config. "
        "Lowercase and hyphens only.{RESET}\n"
    )
    name = _ask("Corpus name", default="my-corpus", validate=_validate_name)
    ...
    if not _yesno("Create this corpus?", default=True):
        print("cancelled.")
        return None

    print(f"\n{BOLD}Creating corpus...{RESET}\n")
    result = scaffold.scaffold(
        name, path, {}, run_bootstrap=True, stream_bootstrap=True
    )

    dest = result["dest"]
    print(f"\n{BOLD}Corpus ready at {dest}{RESET}\n")
    print("Next steps:")
    print(f"  1. cd {dest}")
    ...
    print("     koinome seed <export.json> --staging ./seed-staging")
    return result
