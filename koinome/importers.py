"""Importer wrappers: run one-time seeding against a target corpus.

These operations live in the Koinome package, not inside a corpus folder. Each
wrapper shells out to koinome/importers/, then runs the target corpus's own
...
from .paths import CorpusPathError, resolve_corpus
...
def check_conformance(dest, *, manifest=False, git=False):
    ...
    """
    dest = _dest(dest)
    checker = dest / ".scripts" / "check_corpus.py"
    if checker.is_file():
    ...
    val_rc, val_out, val_err = _run(
        [_PYTHON, ".scripts/validate_corpus.py", "--all"], cwd=dest
    )
    if val_rc != 0:
    ...
    return OK, (val_out or moc_out or "corpus conformance check passed.").strip()


def validate_and_moc(dest):
    """Run the target corpus's MOC generator then validator.
    ...
    return OK, (val_out or val_err or "corpus validation clean.").strip()


def extract_export(export_file, staging, provider=None, since=None, min_messages=None):
    """Stage 1 of seeding: normalize a provider export into a staging area."""
    if not IMPORTERS.is_dir():
        raise ImportError_(
            f"importers not found at {IMPORTERS}. "
            "The installed package is missing importer scripts; reinstall Koinome."
        )
