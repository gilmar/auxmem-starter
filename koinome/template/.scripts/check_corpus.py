#!/usr/bin/env python3
"""Read-only corpus conformance check for humans and CI.
...
def check_validation(root: Path) -> tuple[int, str]:
    rc, out, err = _run([sys.executable, ".scripts/validate_corpus.py", "--all"], cwd=root)
    if rc != 0:
...
def check_manifest(root: Path) -> tuple[int, str]:
    manifest_path = root / ".koinome" / "manifest.json"
    if not manifest_path.is_file():
        return 0, "manifest check skipped (no .koinome/manifest.json)"

    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        return 1, f"manifest check failed: invalid JSON in .koinome/manifest.json ({exc})"
...
def run_checks(
    ...
    *,
    ...
    lines.append("corpus conformance check passed.")
    return 0, lines
