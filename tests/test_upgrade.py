...
from koinome.exit_codes import NON_CONFORMANT, OK, OPERATION_FAILED
from koinome.manifest import verify_bundled_template
from koinome.upgrade import MANIFEST_SRC, TEMPLATE_DIR, PostCheckResult
...
VALID_FM = dict(
    title="Upgrade note",
    summary="Concrete nouns for transactional upgrade tests in this corpus.",
    type="project-doc",
    status="active",
    domain="projects",
    created="2026-07-04",
...
def test_verify_fails_on_missing_template_file(tmp_path):
    ...
    manifest_path = template / ".koinome-manifest.json"
    rel = ".scripts/validate_corpus.py"
    (template / rel).unlink()
...
def test_verify_fails_when_managed_file_missing_from_manifest(tmp_path):
    ...
    manifest_path = template / ".koinome-manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    del manifest["files"][".scripts/validate_corpus.py"]
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    with pytest.raises(ValueError, match="missing from manifest"):
        verify_bundled_template(template, manifest_path)


def test_upgrade_up_to_date(tmp_corpus):
    result = upgrade_mod.upgrade(tmp_corpus)
    assert result["status"] == "up-to-date"


def test_upgrade_force_is_idempotent(tmp_corpus):
    first = upgrade_mod.upgrade(tmp_corpus, force=True)
    assert first["status"] == "upgraded"
    note_path = tmp_corpus / "10-projects" / "seed-note.md"
    if note_path.exists():
        before = note_path.read_bytes()
    second = upgrade_mod.upgrade(tmp_corpus, force=True)
    assert second["status"] == "upgraded"
    if note_path.exists():
        assert note_path.read_bytes() == before


def test_user_notes_unchanged_after_upgrade(tmp_corpus):
    note = write_note(
        tmp_corpus,
    ...
    upgrade_mod.upgrade(tmp_corpus, force=True)
    assert note.read_bytes() == before


def test_dry_run_makes_no_changes(tmp_corpus):
    note = write_note(
        tmp_corpus,
        "10-projects/dry-run.md",
        note_with_fm("Dry run body.", **VALID_FM),
    )
    before_tree = tree_bytes_snapshot(tmp_corpus)
    result = upgrade_mod.upgrade(tmp_corpus, force=True, dry_run=True)
    assert result["status"] == "dry-run"
    assert tree_bytes_snapshot(tmp_corpus) == before_tree
    assert note.read_bytes() == before_tree[str(note.relative_to(tmp_corpus))]


def test_dry_run_cli(tmp_corpus):
    result = run_koinome(["upgrade", "--dry-run", "--force", str(tmp_corpus)])
    assert result.returncode == OK
    assert "dry-run" in result.stdout
    assert "Planned changes" in result.stdout


def test_dry_run_reports_validation_expectation(tmp_corpus):
    result = upgrade_mod.upgrade(tmp_corpus, force=True, dry_run=True)
    assert "post_validation_expected" in result


def test_upgrade_rolls_back_on_post_check_failure(tmp_corpus, monkeypatch):
    tool = tmp_corpus / ".scripts/validate_corpus.py"
...
def test_merge3_conflict_marks_file_and_exits_nonzero(tmp_path, monkeypatch):
    ...
    manifest = json.loads((fake_template / ".koinome-manifest.json").read_text(encoding="utf-8"))
    manifest["files"]["AGENTS.md"]["sha256"] = sha256_file(fake_template / "AGENTS.md")
    (fake_template / ".koinome-manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")

    monkeypatch.setattr(upgrade_mod, "TEMPLATE_DIR", fake_template)
    monkeypatch.setattr(upgrade_mod, "MANIFEST_SRC", fake_template / ".koinome-manifest.json")
...
def test_cli_exits_nonconformant_when_conflicts_remain(tmp_corpus, monkeypatch):
    def fake_upgrade(dest, force=False, dry_run=False):
        return {
            ...
            "conflicts": ["AGENTS.md"],
            "backup": str(tmp_corpus / ".koinome/backups/test"),
    ...
    assert main(["upgrade", "--force", str(tmp_corpus)]) == NON_CONFORMANT


def test_upgrade_preserves_custom_domains(tmp_corpus):
    cfg = read_corpus_config(tmp_corpus)
    domains_before = dict(cfg["domains"])
    upgrade_mod.upgrade(tmp_corpus, force=True)
    cfg_after = read_corpus_config(tmp_corpus)
    assert cfg_after["domains"] == domains_before


def test_upgrade_detects_edited_overwrite_tooling(tmp_corpus):
    tool = tmp_corpus / ".scripts/validate_corpus.py"
    tool.write_text(tool.read_text(encoding="utf-8") + "\n# user patch\n", encoding="utf-8")
    plan = upgrade_mod.build_plan(tmp_corpus, force=True)
    assert any("overwrite: .scripts/validate_corpus.py" in c and "YOUR EDITS" in c for c in plan.changes)
...
def test_sequential_upgrades_safe(tmp_corpus):
    upgrade_mod.upgrade(tmp_corpus, force=True)
    upgrade_mod.upgrade(tmp_corpus, force=True)
    result = upgrade_mod.upgrade(tmp_corpus)
    assert result["status"] == "up-to-date"
